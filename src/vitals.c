...
enum Triage_Level get_vital_triage(struct Vitals vitals) {
  ...
  if (vitals.airway == Blocked || vitals.airway == Inspiratory_stridor ||
      ...
      (vitals.oxygen_saturation < 75 && !vitals.with_oxygen &&
       vitals.with_kol) ||
      vitals.respiration_frequency > 35 || vitals.respiration_frequency < 8 ||
      vitals.pulse > 140 || vitals.systolic_blood_pressure < 80 ||
      vitals.glasgow_coma_scale <= 8 || vitals.temperature_celcius < 32) {
    triage_level = Red;
  } else if ((vitals.oxygen_saturation < 95 && vitals.with_oxygen) ||
             (vitals.oxygen_saturation < 90 && !vitals.with_oxygen) ||
             (vitals.oxygen_saturation < 90 && vitals.with_oxygen &&
              vitals.with_kol) ||
             (vitals.oxygen_saturation < 85 && !vitals.with_oxygen &&
             ...
             vitals.glasgow_coma_scale <= 13 &&
                 vitals.glasgow_coma_scale >= 9 ||
             vitals.temperature_celcius >= 32 &&
                 vitals.temperature_celcius <= 34 ||
